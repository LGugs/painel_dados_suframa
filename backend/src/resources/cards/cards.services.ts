...
import { MaoDeObra } from "./cards.types";
...
//#region MAO DE OBRA SOMATORIO ULTIMO MES - utilizar o mesmo para pegar por polo
export async function maoDeObraCards(
  ...
  conn = await getConnection();
  const result = await conn.execute<MaoDeObra>(
    `SELECT SUM(ima.AMD1_QTDE_TOTAL) TOTAL, SUM(ima.AMD1_QTDE_FEMININA) Feminino, SUM(ima.AMD1_QTDE_PNE) PNE
